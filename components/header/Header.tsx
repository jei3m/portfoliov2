      ...
      <nav className='flex items-center justify-between px-4 sm:px-6'>
        ...
        <ul className='flex items-center gap-6 text-sm font-light sm:gap-10'>
          ...
          {navLinks.map((link, index) => (
            <li key={index} className='transition-colors hover:text-foreground sm:hidden'>
          ...
          {navLinks.map((link, index) => (
            <li key={index} className='transition-colors hover:text-foreground text-yellow-50 hidden sm:block'>
              <TransitionLink href={link.href}>{link.text}</TransitionLink>
            </li>
          ))}
        </ul>
      </nav>
